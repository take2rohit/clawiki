...
paths: "raw/**, */papers/**, */index.md, */log.md, bibtex/**"
---
...
## Branch-Named Wiki Directory

**The wiki directory is always named after the current git branch.** For example, on branch `world_models`, the wiki lives in `world_models/`. On branch `jepa_research`, it lives in `jepa_research/`.

To get the wiki directory path in any skill:
```bash
BRANCH=$(git branch --show-current)
# Wiki root:    $BRANCH/
# Papers:       $BRANCH/papers/
# Index:        $BRANCH/index.md
# Log:          $BRANCH/log.md
```

In `_config.yml`, the `branch` key stores the current branch name for Jekyll to use in templates (e.g., root `index.md` redirect).

Throughout this schema and all skills, `$BRANCH/` refers to this branch-named directory.
...
**Wiki pages:** Same base name as the PDF: `$BRANCH/papers/{author}-{year}-{venue}.md`

## Index as Single Source of Truth (`$BRANCH/index.md`)

`$BRANCH/index.md` is the **only** place that tracks all papers and their status. There is no CSV or database.
...
**Link format is universal.** `../raw/` and `papers/{slug}.md` resolve correctly in Obsidian (vault at repo root), GitHub.com (viewing $BRANCH/index.md), and GitHub Pages (Jekyll serves $BRANCH/index.md at /$BRANCH/). Never change this format — no rewriting needed for hosting.
...
4. **After every ingest**: Grep the branch directory for mentions of the paper's authors/methods/terms
...
## Log Format (`$BRANCH/log.md`)

Append-only. `$BRANCH/log.md` has YAML frontmatter. Each entry is a bullet with the date inline. Parseable with `grep "^\- \[" $BRANCH/log.md | tail -10`.
...
```bash
BRANCH=$(git branch --show-current)
echo "- [$(date "+%Y-%m-%d %H:%M")] **verb** -	details" >> $BRANCH/log.md
```
...
All wiki content is hosted via `/host` on a topic branch with Jekyll (`jekyll-theme-slate`). The wiki directory URL matches the branch name (e.g., branch `world_models` → `/clawiki/world_models/`). Two rules keep everything compatible:
...
2. **Every operation updates `$BRANCH/index.md` and appends to `$BRANCH/log.md`.**
3. **`$BRANCH/index.md` is the single source of truth** for paper status and catalog.
